...
#GRUB
BOOTABLEISOFILENAME = kernel.iso
#must contain boot/grub/grub.cfg
ISODIR = iso
ISOPATHTOGRUB = $(ISODIR)
...
$(KERNELBINFILENAME): $(OBJDIR) $(ASMOBJFILES) $(COBJFILES)
	$(UTILSPATH)/$(LINKER) -T $(LINKERSCRIPTFILE) -o $(KERNELBINFILENAME) $(OBJDIR)/$(ASMOBJFILESNAMES) $(OBJDIR)/$(COBJFILESNAMES) $(LINKERFLAGS)
